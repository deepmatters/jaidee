    ...
    .then(data => {
        ...
        data.forEach(i => {
            if (i.result == 0) {  // If no result
                ...
                reqRes.push(
                    `เพื่อน${i.gender} อายุ ${i.age} จากจังหวัด${i.area} บอกว่า${pronoun}มีปัญหาเรื่อง <span class='response-topic'>"${i.topic}"</span> ซึ่ง${pronoun}แก้ไขโดย <span class='response-solution'>"${i.solution}"</span>`
                )
            }
        })
...
const regexSuicide = new RegExp('อยากตาย|ฆ่าตัวตาย|ไม่อยากอยู่|จบชีวิต', 'g')
...
function convStart() {
    ...
    const resGreeting = [
        ...
        `สวัสดีจ๊ะ พี่${botName}เป็นแชทบอทช่วยน้องหาทางออกเรื่องกลุ้มใจ`, 
        `ไม่ว่าจะเป็นเรื่องอะไร พอเธอพิมพ์มาถามพี่ พี่จะช่วยดูให้ว่าเพื่อนที่เคยผ่านเรื่องนั้นมาแล้ว เขาทำยังไงกันบ้าง<br><span class="help-text">พี่จะไม่รู้และไม่ถามว่าเธอเป็นใคร และจะ <a href="/about" target="_blank">เก็บความลับ</a> ของเธอเป็นอย่างดี</span>`, 
        `
...
function convProcess(reqInput) {
    ...
    if (reqInput == 'เริ่มคุย' || 
        ...
        reqRes = [
            'ดีใจที่มาคุยกัน', 
            `เธอกำลังกลุ้มใจเรื่องอะไรอยู่ เล่าให้พี่${botName}ฟังได้นะ ลองพิมพ์ปัญหาเป็นคำหรือประโยคสั้นๆ นะ<br><span class="help-text">ลองพิมพ์ <em>แอบชอบรุ่นพี่</em>, <em>เรียนไม่รู้เรื่อง</em>, <em>พ่อแม่ไม่เข้าใจ</em></span>`
        ]
...
function convAddIntervention(flag) {
    ...
    setTimeout(function() {
        ...
        if (flag == 'suicide') {
            document.getElementById(`${convNum+1}`).innerHTML = `    
                พี่${botName}รับฟังเธอนะ ถ้าเธอมีความคิดว่าไม่อยากมีชีวิตอยู่ มีหลายหน่วยงานยินดีช่วยเหลือเธอนะ ลองโทรสายด่วนกรมสุขภาพจิต เบอร์ <strong>1323</strong> ได้ตลอดเวลา พี่เป็นกำลังใจให้เสมอ
            `
        }
...
function convAddNotice() {
    ...
    setTimeout(function() {
        ...
        botConv.appendChild(childNode)
        document.getElementById(`${convNum+1}`).innerHTML = `
            พี่${botName}ยังไม่มีข้อมูลที่ตรงร้อยเปอร์เซ็นกับที่เธอพิมพ์มา เลยลองหาเรื่องที่อาจจะเกี่ยวข้องมาให้ ถ้าดูแล้วไม่ตรง ลองพิมพ์ใหม่โดยเปลี่ยนคำหรือใช้ประโยคที่สั้นลงนะคะ
        `
